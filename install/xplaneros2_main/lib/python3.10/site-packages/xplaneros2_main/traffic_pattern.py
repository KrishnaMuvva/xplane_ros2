...
class Traffic_Pattern(Node):
	...
	def CrossWind(self):
		...
		if abs(self.uav_local_state.local_z - self.initial_z) > 18000:
			self.fsm_current_state += 1
	...
	def DownWind(self):
		...
		if self.xcoord_negative:
			if self.uav_local_state.local_x > (self.initial_x+30000):
				self.fsm_current_state += 1
		else:
			if self.uav_local_state.local_x < (self.initial_x-30000):
				self.fsm_current_state += 1	
	...
	def BaseLeg(self):
		...
		if abs(self.uav_local_state.local_z - self.initial_z) < 12000:
			self.fsm_current_state += 1

	def FinalLeg(self):

		self.uav_autopilot.heading, self.uav_autopilot.airspeed, self.uav_autopilot.vertical_velocity = self.yaw_angles[self.fsm_current_state], self.desired_airspeeds[self.fsm_current_state], -200.0
