...
class Xplane_ROS2(Node):
	...
	def Takeoff(self):
		...
		if self.uav_state.airspeed < self.min_takeoff_speed:
			...
			print("Trying to achieve takeoff speed ", self.uav_state.airspeed, self.min_takeoff_speed)
		...
		if self.uav_state.altitude < self.cruise_alt_goal:
			...
			print("Trying to achieve goal altitude ", self.uav_state.altitude, self.cruise_alt_goal)
